...
// host and port.
func NewTCP(host string) (*tcpClient, error) {
	client := &tcpClient{
		host:     host,
...
func (c *tcpClient) Subscribe(tags []string) error {
...
func (c *tcpClient) Unsubscribe(tags []string) error {
...
// clients
func (c *tcpClient) Publish(tags []string, data string) error {
...
func (c *tcpClient) PublishAfter(tags []string, data string, delay time.Duration) error {
	go func() {
		<-time.After(delay)
		c.Publish(tags, data)
	}()
...
//
func (c *tcpClient) Messages() <-chan mist.Message {
	return c.messages
}
